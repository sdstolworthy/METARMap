...
POLL_FREQUENCY_IN_SECONDS = 10 * 60
...
pixels = neopixel.NeoPixel(
    LED_PIN, LED_COUNT, brightness=LED_BRIGHTNESS, pixel_order=LED_ORDER, auto_write=False)
...
while True:
    ...
    for metar in root.iter('METAR'):
        ...
        if metar.find('wind_gust_kt') is not None:
            windGust = (True if (ALWAYS_BLINK_FOR_GUSTS or int(
                metar.find('wind_gust_kt').text) > WIND_BLINK_THRESHOLD) else False)
